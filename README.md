...
audio.cpp 是一个纯 C++ 音频推理框架，基于 ggml，支持 TTS、STT（ASR）、VAD、说话人分离、音源分离、声音克隆、编解码与音乐生成等 91 个 loader 族任务，后端覆盖 CPU / CUDA / HIP / Vulkan / Metal，可加载 GGUF 与 safetensors 模型。
...
| `full-models` | `AUDIOCPP_MODEL_SET=full` | **全量**：audio.cpp 全部 91 个 loader 族（编译极慢、产物很大，权重仍需自行下载） |
...
- 其余未列出的模型族（roformer、htdemucs、outetts、glm_tts、pocket_tts、
  ...
  另注意 `ModelFamily` 的覆盖范围大于 `model-*` feature（枚举收录全部上游
  loader 族、未知族可用 `Custom(String)` 兜底），未列出族的加载提示仍可用族名。
