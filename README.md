...
audio.cpp 是一个纯 C++ 音频推理框架，基于 ggml，支持 TTS、STT（ASR）、VAD、说话人分离、音源分离、声音克隆、编解码与音乐生成等 87 个 loader 族任务，后端覆盖 CPU / CUDA / HIP / Vulkan / Metal，可加载 GGUF 与 safetensors 模型。
...
| `full-models` | `AUDIOCPP_MODEL_SET=full` | **全量**：audio.cpp 全部 87 个 loader 族（编译极慢、产物很大，权重仍需自行下载） |
